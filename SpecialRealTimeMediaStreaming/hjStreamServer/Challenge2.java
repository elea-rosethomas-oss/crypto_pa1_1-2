...
class Challenge2 {
    ...
    static public void main( String []args ) throws Exception {
        ...
        byte[] key = "streaming_key_32streaming_key_32".getBytes();	//Key for Chacha encryption 32 bytes
        SecretKeySpec skeySpec = new SecretKeySpec(key, "ChaCha20"); // same as aes gcm but with chacha
        ...
        while ( g.available() > 0 ) {
            ...
            //real encryption befrore puting in block

            byte[] iv = generateNonce(); //cf fun
            cipher.init(Cipher.ENCRYPT_MODE, skeySpec, new IvParameterSpec(iv));
            byte[] encryptedBytes = cipher.doFinal(buff, 0, size);

            int totalSize = iv.length + encryptedBytes.length;
            byte[] finalbuff = new byte[totalSize];
            System.arraycopy(iv, 0, finalbuff, 0, iv.length);
            System.arraycopy(encryptedBytes, 0, finalbuff, iv.length, encryptedBytes.length);

            p.setData(finalbuff, 0, totalSize);
