...
class hjStreamServerSHP {

	static public void main( String []args ) throws Exception {
	        if (args.length != 3 && args.length != 4)
	        {
                   System.out.println("Erro, usar: mySend <movie> <ip-multicast-address> <rtssp-port> [shp-port]");
	           System.out.println("        or: mySend <movie> <ip-unicast-address> <rtssp-port> [shp-port]");
	           System.exit(-1);
                }
		...
		int shpPort = args.length == 4 ? Integer.parseInt(args[3]) : 9999;
		long handshakeStart = System.nanoTime();
		SHPProtocol.SessionKeys sessionKeys = performSecureHandshake(movieName, shpPort);
		long handshakeEnd = System.nanoTime();
		CryptoConfig cryptoConfig = CryptoConfig.fromSession(movieName + ".encrypted", sessionKeys);
		System.out.println("SHP ready: " + sessionKeys.ciphersuite + " for " + movieName);
		...
		writeServerStats("hjStreamServerSHP.stats.log", args[0], addr.toString(), cryptoConfig.name,
				cryptoConfig.ciphersuite, count, segmentsSent, sendFailures, csize, encryptedBytes, t0, tend,
				handshakeStart, handshakeEnd, shpPort);
	...
	private static void writeServerStats(String logFile, String movie, String destination, String configName,
										 ...
										 long handshakeStartNanos, long handshakeEndNanos, int shpPort)
			throws IOException {
		...
		out.println("=== SHP Stream Server Stats " + LocalDateTime.now() + " ===");
		out.println("movie=" + movie);
		out.println("destination=" + destination);
		out.println("shp_port=" + shpPort);
	...
	private static SHPProtocol.SessionKeys performSecureHandshake(String expectedMovie, int shpPort) throws Exception {
		DatagramSocket socket = new DatagramSocket(shpPort);
		byte[] buffer = new byte[8192];

		DatagramPacket clientPacket = new DatagramPacket(buffer, buffer.length);
		socket.receive(clientPacket);
		byte[] clientHelloBytes = Arrays.copyOf(clientPacket.getData(), clientPacket.getLength());
		SHPProtocol.ClientHello clientHello = SHPProtocol.parseClientHello(clientHelloBytes);
		if (!expectedMovie.equals(clientHello.movie)) {
			throw new GeneralSecurityException("SHP ClientHello requested " + clientHello.movie + " but server is streaming " + expectedMovie);
		}
		String selectedCiphersuite = SHPProtocol.chooseCiphersuite(clientHello.ciphersuites);
		if (selectedCiphersuite == null) {
			throw new GeneralSecurityException("SHP ClientHello did not offer a supported ciphersuite");
		}

		KeyPair identity = SHPProtocol.generateEcKeyPair();
		KeyPair ecdhKeyPair = SHPProtocol.generateEcKeyPair();
		SHPProtocol.Certificate certificate = SHPProtocol.createCertificate("streaming-server", identity);
		byte[] serverNonce = SHPProtocol.randomNonce();
		SHPProtocol.ServerHello serverHello = SHPProtocol.createServerHello(expectedMovie, true, certificate,
				ecdhKeyPair, selectedCiphersuite, serverNonce, SHPProtocol.challengeResponse(clientHello.nonce), identity.getPrivate());
		byte[] serverHelloBytes = SHPProtocol.encodeServerHello(serverHello);
		socket.send(new DatagramPacket(serverHelloBytes, serverHelloBytes.length, clientPacket.getSocketAddress()));

		SHPProtocol.SessionKeys sessionKeys = SHPProtocol.deriveSessionKeys(ecdhKeyPair.getPrivate(), clientHello.ecdhPublicKey,
		...
		SHPProtocol.CsspPlaintext cssp = SHPProtocol.parseCssp(Arrays.copyOf(csspPacket.getData(), csspPacket.getLength()), sessionKeys);
		if (!expectedMovie.equals(cssp.movie) || !"START_RTSSP".equals(cssp.command)) {
			throw new GeneralSecurityException("Invalid SHP CSSP command");
		}
		if (!MessageDigest.isEqual(SHPProtocol.challengeResponse(serverNonce), cssp.serverChallengeResponse)) {
			throw new GeneralSecurityException("SHP proxy failed the server nonce challenge");
		}
		socket.close();
		return sessionKeys;
	}
